...
export async function GET() {
  ...
  const providers = [
    ...
    { id: 'embed', label: 'Gemini embeddings (recall)', kind: 'model' as const, configured: boolEnv('GEMINI_API_KEY'), usageNote: 'Tied to same key as text model.' },
    ...
    { id: 'postgres', label: 'Local PostgreSQL (DB + auth)', kind: 'platform' as const, configured: boolEnv('DATABASE_URL'), usageNote: 'Local Postgres via DATABASE_URL.' },
  ];
