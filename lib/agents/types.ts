...
export interface OrchestratorOutput {
  ...
  /**
   * Market briefing for companies this workspace already holds a history on.
   *
   * Attached after the agents run, not by them. When someone names a company we
   * have eight months of collected history for, the answer should include the
   * share, the trend and the timeline we already computed — going back out to
   * the live web for facts we hold is slower, costs money, and returns less.
   * Absent when the query names nobody we follow.
   */
  marketBriefing?: MarketBriefingPayload;
  generatedAt: string;
...
/**
 * A market briefing carried on an orchestrator result.
 *
 * Structurally a subset of `lib/market/briefing.ts`'s `Briefing`, restated here
 * so `lib/agents` does not depend on the market dataset — the agents must keep
 * working unchanged when the briefing is absent, which is every query about a
 * company we do not follow.
 */
export interface MarketBriefingPayload {
  ...
  readOut: string;
  months: string[];
  /** Only the companies the query actually named. */
  companies: Array<{
    label: string;
    what: string;
    share: number[];
